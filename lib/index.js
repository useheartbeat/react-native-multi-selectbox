...
const kOptionsHeight = { width: '100%'}
...
function HeaderComponent({
  ...
  inputFilterStyle,
  filterInputPlaceholderTextColor,
  searchInputProps,
  searchIconColor
}) {
  ...
  return (
    <>
      {!hideInputFilter && !searchInputComponent && (
        <View style={kInputFilterContainerStyle}>
          <TextInput
            ...
            placeholderTextColor={filterInputPlaceholderTextColor}
            {...searchInputProps}
          />
      ...
      {searchInputComponent && (
        searchInputComponent(onChangeText(), inputValue)
      )}
...
function SelectBox({
  ...
  const [showOptions, setShowOptions] = useState(shouldShowOptions)
  ...
  function renderItem({ item, index }) {
    ...
    return selectionItemComponent ? (selectionItemComponent(item, index, onPressMultiItem, renderItemStyle))
    : (
  ...
  function renderGroupItem({ item }) {
    ...
    return (
      <View style={kMultiOptionContainerStyle}>
        <Text style={kMultiOptionsLabelStyle}>{label?.item || item.item}</Text>
  ...
  const SectionData = inputValue !== "" ? [{title: `${filteredSuggestions.length} results`, data: filteredSuggestions}] : [{title: "Most Common", data: defaultOptions}]
    ...
    <>
      <View>
        <HeaderComponent
          ...
          filterInputPlaceholderTextColor={filterInputPlaceholderTextColor}
  ...
  function onPressShowOptions() {
    return () => onPress && !showOptions ? onPress() : setShowOptions(!showOptions)
  }
}
